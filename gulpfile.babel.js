...
const watchSource = {
  ...
  php: [
    `!(${CFG.FOLDER_VENDOR}*)/**/*`, PATH_SRC + '**/*.php', PATH_SRC + '**/.htaccess', PATH_SRC + '**/*.xml', PATH_SRC + '**/*.txt'
  ],
  fonts: {
...
const __php = {
  dev: (cb) => {
    src([PATH_SRC + '**/*.php', PATH_SRC + '**/.htaccess', PATH_SRC + '**/*.xml', PATH_SRC + '**/*.txt'])
      .pipe(dest(PATH_DEV));
...
const __static = {
  dev: (cb) => {
    src(PATH_SRC + CFG.FOLDER_STATIC + '**/*')
      .pipe(dest(PATH_DEV));
    progressBar.increment();
    cb();
  },
